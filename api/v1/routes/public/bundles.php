...
if ($first === 'bundles') {
    ...
    if ($bundleId) {
        try {
            // 🔒 SECURITY: Enforce tenant isolation via JOIN on entities
            $bundle = $pdoOne(
                "SELECT b.id, b.entity_id, b.bundle_name, b.bundle_name_ar,
                  ...
                  WHERE b.id = ? AND b.is_active = 1 AND e.tenant_id = ?",
                [$bundleId, $tenantId]
            );
        ...
        } catch (ApplicationException|\RuntimeException $ex) { ResponseFormatter::error($ex->getMessage(), 500); }
        exit;
    }
    ...
    if (!empty($_GET['entity_id'])) {
        ...
        if ($eId > 0 && class_exists('MultiTenantValidator')) {
            if (!MultiTenantValidator::checkOwnership($pdo, 'entities', $eId, $tenantId)) {
