...
try {
    ...
    foreach ($pathParts as $i => $part) {
        if ($part === 'discount_scopes' && isset($pathParts[$i + 1]) && ctype_digit($pathParts[$i + 1])) {
    ...
    switch ($method) {
        case 'GET':
            ...
            ResponseFormatter::success($controller->listScopes($discountId));
            break;

        case 'POST':
            $data = json_decode(file_get_contents('php://input'), true) ?: [];
            $data = array_intersect_key($data, array_flip(['discount_id', 'scope_type', 'scope_id']));

            $validation = DiscountsValidator::validateScope($data);
            if (!$validation['valid']) {
    ...
    safe_log('warning', 'discount_scopes.validation', ['error' => $e->getMessage()]);
    ResponseFormatter::error($e->getMessage(), 422);
} catch (\PDOException $e) {
    safe_log('critical', 'discount_scopes.db_error', ['error' => $e->getMessage()]);
    ResponseFormatter::error('Database error', 500);
} catch (\RuntimeException $e) {
    safe_log('error', 'discount_scopes.runtime', ['error' => $e->getMessage()]);
