...
try {
    ...
    foreach ($pathParts as $i => $part) {
        if ($part === 'discount_conditions' && isset($pathParts[$i + 1]) && ctype_digit($pathParts[$i + 1])) {
    ...
    switch ($method) {
        case 'GET':
            ...
            ResponseFormatter::success($controller->listConditions($discountId));
            break;

        case 'POST':
            $data = json_decode(file_get_contents('php://input'), true) ?: [];
            $data = array_intersect_key($data, array_flip(['discount_id', 'condition_type', 'operator', 'condition_value']));
            
            $validation = DiscountsValidator::validateCondition($data);
            if (!$validation['valid']) {
        ...
        case 'PUT':
            ...
            $data = array_intersect_key($data, array_flip(['condition_type', 'operator', 'condition_value']));
            if (empty($data)) {
    ...
    safe_log('warning', 'discount_conditions.validation', ['error' => $e->getMessage()]);
    ResponseFormatter::error($e->getMessage(), 422);
} catch (\PDOException $e) {
    safe_log('critical', 'discount_conditions.db_error', ['error' => $e->getMessage()]);
    ResponseFormatter::error('Database error', 500);
} catch (\RuntimeException $e) {
    safe_log('error', 'discount_conditions.runtime', ['error' => $e->getMessage()]);
