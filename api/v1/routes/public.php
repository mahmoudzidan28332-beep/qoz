...
$offset   = ($page - 1) * $per;
$tenantIdFromGet = isset($_GET['tenant_id']) && is_numeric($_GET['tenant_id'])
                   ? (int)$_GET['tenant_id']
                   : null;

// 🔒 SECURITY: checkOwnership For authenticated users, enforce their assigned tenant_id (unless super_admin)
$sessionTenantId = (int)($_SESSION['tenant_id'] ?? 0);
$isSuperAdmin    = in_array('super_admin', $_SESSION['user']['roles'] ?? [], true);

if ($sessionTenantId > 0 && !$isSuperAdmin) {
    if ($tenantIdFromGet !== null && $tenantIdFromGet !== $sessionTenantId) {
        ResponseFormatter::error('Unauthorized for this tenant', 403);
        exit;
    }
    $tenantId = $sessionTenantId;
} else {
    $tenantId = $tenantIdFromGet;
}
