...
try {
    ...
    foreach ($pathParts as $i => $part) {
        if ($part === 'discount_exclusions' && isset($pathParts[$i + 1]) && ctype_digit($pathParts[$i + 1])) {
    ...
    switch ($method) {
        case 'GET':
            ...
            ResponseFormatter::success($controller->listExclusions($discountId));
            break;

        case 'POST':
            $data = json_decode(file_get_contents('php://input'), true) ?: [];
            $data = array_intersect_key($data, array_flip(['discount_id', 'excluded_discount_id']));

            $validation = DiscountsValidator::validateExclusion($data);
            if (!$validation['valid']) {
                ResponseFormatter::error('Validation failed: ' . implode(', ', $validation['errors']), 422);
                break;
            }
            $id = $controller->createExclusion(
                (int)$data['discount_id'],
                (int)$data['excluded_discount_id']
            );
    ...
    safe_log('warning', 'discount_exclusions.validation', ['error' => $e->getMessage()]);
    ResponseFormatter::error($e->getMessage(), 422);
} catch (\PDOException $e) {
    safe_log('critical', 'discount_exclusions.db_error', ['error' => $e->getMessage()]);
    ResponseFormatter::error('Database error', 500);
} catch (\RuntimeException $e) {
    safe_log('error', 'discount_exclusions.runtime', ['error' => $e->getMessage()]);
