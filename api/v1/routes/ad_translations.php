...
require_once $modelsPath . '/Contracts/AdTranslationsRepositoryInterface.php';
require_once $modelsPath . '/repositories/PdoAdTranslationsRepository.php';
require_once $modelsPath . '/validators/AdTranslationsValidator.php';
require_once $modelsPath . '/services/AdTranslationsService.php';
require_once $modelsPath . '/controllers/AdTranslationsController.php';
...
if ($isPlatformAdmin && $tenantId > 0 && class_exists('PlatformContext', false)) {
    PlatformContext::logCrossTenantAction(
        ...
        reason: 'Platform Admin — ad_translations management'
    );
}

try {
    $repo       = new PdoAdTranslationsRepository($pdo);
    $service    = new AdTranslationsService($repo);
    $controller = new AdTranslationsController($service);
    ...
    switch ($method) {
        ...
        case 'POST':
            ...
            ResponseFormatter::success(['id' => $newId], 'Ad translation created successfully', 201);
            break;

        case 'PUT':
            $data      = json_decode(file_get_contents('php://input'), true) ?: [];
            $updatedId = $controller->update($tenantId, $data);
            ResponseFormatter::success(['id' => $updatedId], 'Ad translation updated successfully');
            break;
        ...
        case 'DELETE':
            ...
            ResponseFormatter::success(['deleted' => $deleted], 'Ad translation deleted successfully');
            break;
    ...
    safe_log('warning', 'ad_translations.validation', ['error' => $e->getMessage()]);
    ResponseFormatter::error($e->getMessage(), 422);
} catch (DatabaseException|\PDOException $e) {
    safe_log('error', 'ad_translations.database', [
        'code'  => $e->getCode(),
        'error' => $e->getMessage(),
    ...
    safe_log('error', 'ad_translations.runtime', ['error' => $e->getMessage()]);
