...
final class TenantBootstrapService
{
    ...
    private function upsertGlobalPermissions(): array
    {
        ...
        $stmt = $this->pdo->prepare(
            "SELECT id, key_name FROM permissions
             WHERE tenant_id = ? AND key_name IN ({$placeholders})"
        );
        $stmt->execute(array_merge([$systemTenantId], $keys));
    ...
    /**
     * Insert the three default tenant-scoped roles.
     *
    ...
    private function createDefaultRoles(int $tenantId): array
    {
        ...
        foreach ($definitions as $key => $displayName) {
            $stmt->execute([$tenantId, $key, $displayName]);
            $ids[$key] = (int)$this->pdo->lastInsertId();
        }
