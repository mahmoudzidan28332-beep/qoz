...
final class PdoNotificationChannelsRepository
{
    private PDO $pdo;

    private const ALLOWED_ORDER_BY = ['id', 'code', 'name', 'is_active', 'created_at'];
    private const FILTERABLE_COLUMNS = ['code', 'name', 'is_active'];
    ...
    {
        $sql    = "SELECT COUNT(*) FROM notification_channels WHERE 1=1";
        $params = [];
    ...
    public function save(array $data): int
    {
        ...
        if ($isUpdate) {
            ...
            $sql  = "UPDATE notification_channels SET " . implode(', ', $sets) . " WHERE id = :id";
            $stmt = $this->pdo->prepare($sql);
            $stmt->execute($params);
            return $id;
        }
        ...
        $sql  = "INSERT INTO notification_channels (" . implode(', ', $cols) . ") VALUES (" . implode(', ', $placeholders) . ")";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute($params);
