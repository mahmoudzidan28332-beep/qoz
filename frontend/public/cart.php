...
(function () {
  ...
  function renderCart(cartItems) {
    ...
    cartItems.forEach(function (item, idx) {
      ...
      var sale    = parseFloat(item.sale_price) || 0;
      var displayPrice = (sale > 0 && sale < price) ? sale : price;
      
      var qty     = Math.max(1, parseInt(item.qty, 10) || 1);
      var subtotal = displayPrice * qty;
      total    += subtotal;
      itemCount += qty;
      var dbId  = item._db_item_id || 0;

      var img = item.image ? '<img src="' + esc(imgUrl(item.image)) + '" alt="' + esc(item.name) + '" loading="lazy" onerror="this.style.display=\'none\'">' : '🖼️';
      
      var priceHtml = (sale > 0 && sale < price) 
        ? '<span class="pub-cart-price-original">' + price.toFixed(2) + '</span> <span class="pub-cart-price-sale">' + sale.toFixed(2) + ' ' + esc(CURRENCY) + '</span>'
        : '<span>' + price.toFixed(2) + ' ' + esc(CURRENCY) + '</span>';
      ...
      div.innerHTML =
        ...
        + '<a href="/frontend/public/product.php?id=' + (parseInt(item.id, 10)||0) + '" class="pub-cart-item-name">' + esc(item.name || '') + '</a>'
        + '<div class="pub-cart-item-price">' + priceHtml + '</div>'
        + '</div>'
        ...
        + '<input type="number" class="pub-qty-input cart-qty-inp" value="' + qty + '" min="1" max="999" data-idx="' + idx + '" data-db-id="' + dbId + '" data-price="' + displayPrice + '">'
        + '<button class="pub-qty-btn" type="button" data-idx="' + idx + '" data-db-id="' + dbId + '" data-delta="1">+</button>'
        + '</div>'
  ...
  function dbUpdate(itemId, qty, cb) {
    ...
    fetch('/api/public/cart/update?tenant_id=' + TENANT_ID + '&_t=' + Date.now(), {
      method: 'POST',
  ...
  function dbRemove(itemId, cb) {
    if (!itemId) return cb && cb();
    fetch('/api/public/cart/remove?tenant_id=' + TENANT_ID + '&_t=' + Date.now(), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
      credentials: 'include',
  ...
  function loadCart() {
    ...
    fetch('/api/public/cart?tenant_id=' + TENANT_ID + '&_t=' + Date.now(), {
      credentials: 'include',
      headers: { 'Accept': 'application/json' }
    })
    .then(function(r) { return r.ok ? r.json() : Promise.reject(r.status); })
    .then(function(resp) {
      var dbItems = (resp.data && resp.data.items) ? resp.data.items : [];
      var localCart = getLocalCart();
      
      // Map DB items to our display format
      var remoteItems = dbItems.map(function(ci) {
        return {
          id: parseInt(ci.product_id, 10),
      ...
      // Merge: Keep all remote items, and add local items that are NOT in remote
      var hasRemoteCart = !!(resp.data && resp.data.cart);
      var merged = remoteItems.slice();

      // For logged-in users with a DB cart, we completely ignore localStorage metadata 
      // to prevent "cache pollution" from old or different sessions.
      // We only add local items if we really want to support guest-to-member merging, 
      // but since the user is seeing "wrong data", we'll stick to DB truth.

      _cartItems = merged;

      // TRUST DB if it has an active cart record.
      if (hasRemoteCart) {
          // If we have an active cart in DB, its items are the source of truth.
          _cartItems = remoteItems; 
          saveLocalCart(_cartItems.map(function(ci) {
            return { id: ci.id, name: ci.name, price: ci.price, qty: ci.qty, sku: ci.sku, image: ci.image };
          }));
      } else if (resp.data) {
          // The API successfully responded and confirmed there is NO active cart in the DB.
          // Since all 'Add to Cart' actions require login and sync to DB, 
          // any items in localStorage for a logged-in user with no DB cart are stale "ghosts".
          _cartItems = [];
          saveLocalCart([]);
      }
      // Else: API returned an error or unexpected format. 
      // We keep _cartItems as is (merged) but DON'T wipe localStorage to stay safe.
    ...
    .catch(function() {
      ...
      _cartItems = getLocalCart().map(function(i) {
        var lid = parseInt(i.id, 10);
        return Object.assign({}, i, { id: lid, _db_item_id: 0 });
      });
      if (loading) loading.style.display = 'none';
      renderCart(_cartItems);
    });
