(function () {
    ...
    async function loadProducts(page = 1) {
        try {
            ...
            if (result.success && result.data) {
                ...
                const total = result.data.total !== undefined ? result.data.total : (result.meta?.total || items.length);

                state.products = Array.isArray(items) ? items : [];
                state.total = total;

                await renderTable(state.products);
                updatePagination({ page, per_page: state.perPage, total: state.total });
    ...
    async function renderTable(items) {
        ...
        const isSuperAdmin = state.permissions.isSuperAdmin || state.permissions.isPlatformAdmin;
    ...
    function resetFilters() {
        ...
        if (el.tenantFilter) el.tenantFilter.value = (state.permissions.isSuperAdmin || state.permissions.isPlatformAdmin) ? '0' : state.tenantId;
    ...
    async function init() {
        ...
        if (el.tenantFilter) {
            el.tenantFilter.onchange = applyFilters;
            el.tenantFilter.onkeyup = function(e) { if(e.key === 'Enter') applyFilters(); };
        }
