...
(function () {
    ...
    /**
     * Helper to get tenant_id for API calls. 
     * Respects Platform Admin (null/undefined) vs Tenant Admin (integer).
     */
    function getTenantIdForApi() {
        if (!window.APP_CONFIG) return 0;
        if (window.APP_CONFIG.TENANT_ID === undefined || window.APP_CONFIG.TENANT_ID === null) {
            return 0;
        }
        return window.APP_CONFIG.TENANT_ID;
    }
    ...
    async function loadParents() {
        try {
            ...
            const params = new URLSearchParams({
                show_all: '1',
                limit: 0,
                tenant_id: getTenantIdForApi(),
                lang: getDataLanguage(),
                format: 'json'
            });
    ...
    function normalizeTranslationsForSave(translations, baseData) {
        const normalizedMap = new Map();

        const rows = Array.isArray(translations)
            ? translations
            : (translations && typeof translations === 'object'
                ? Object.entries(translations).map(([languageCode, translation]) => ({
                    ...
                    language_code: translation?.language_code || languageCode
                }))
                : []);

        rows.forEach((translation) => {
            if (!translation || typeof translation !== 'object') return;

            const languageCode = String(translation.language_code || '').trim().toLowerCase();
            if (!languageCode) return;

            normalizedMap.set(languageCode, {
                ...
                meta_keywords: String(translation.meta_keywords || '').trim()
            });
        });

        const defaultTranslation = normalizedMap.get('en') || {
            language_code: 'en',
            name: '',
            slug: '',
            description: '',
        ...
        defaultTranslation.name = defaultTranslation.name || String(baseData.name || '').trim();
        defaultTranslation.slug = defaultTranslation.slug || String(baseData.slug || '').trim();
        defaultTranslation.description = defaultTranslation.description || String(baseData.description || '').trim();
        normalizedMap.set('en', defaultTranslation);

        const primary = normalizedMap.get('en');
        return {
            ...baseData,
            name: primary.name || String(baseData.name || '').trim(),
            slug: primary.slug || String(baseData.slug || '').trim(),
            description: primary.description || String(baseData.description || '').trim(),
            translations: Array.from(normalizedMap.values())
        };
    }

    function syncDefaultTranslationPanel(force = false) {
        const panel = el.translations?.querySelector('[data-lang="en"]');
        if (!panel) return;

        const fieldMap = {
            name: el.formName?.value || '',
            slug: el.formSlug?.value || '',
            description: el.formDescription?.value || ''
        };

        Object.entries(fieldMap).forEach(([field, value]) => {
            const input = panel.querySelector(`[name="translations[en][${field}]"]`);
            if (!input) return;
    ...
    async function save(e) {
        ...
        let tenant_id = getTenantIdForApi();
        if (tenant_id === '' && !window.PAGE_PERMISSIONS?.isSuperAdmin) tenant_id = 1;
        if (window.PAGE_PERMISSIONS?.isSuperAdmin || (window.ADMIN_UI && window.ADMIN_UI.is_super_admin)) {
            if (formData.tenant_id === '' || formData.tenant_id === undefined) {
                tenant_id = null;
            } else {
                tenant_id = parseInt(formData.tenant_id);
                if (isNaN(tenant_id)) tenant_id = null;
            }
        }
        ...
        data = normalizeTranslationsForSave(data.translations, data);
        data.deleted_translations = deletions;
    ...
    async function edit(id) {
        ...
        try {
            const response = await AF.get(`${API}/${id}?format=json&lang=${getDataLanguage()}&tenant_id=${getTenantIdForApi()}&all_translations=1`);
            const { payload } = normalizeApiResponse(response);
            ...
            const defaultTranslation = Array.isArray(item.translations)
                ? item.translations.find(tr => tr && tr.language_code === 'en')
                : (item.translations && typeof item.translations === 'object' ? item.translations.en : null);

            el.formId.value = String(item.id || '');
            el.formName.value = defaultTranslation?.name || item.name || '';
            el.formSlug.value = defaultTranslation?.slug || item.slug || '';
            ...
            el.formDescription.value = defaultTranslation?.description || item.description || '';
            ...
            let tenantId = getTenantIdForApi();
            if (tenantId === '' && !window.PAGE_PERMISSIONS?.isSuperAdmin) tenantId = 1;
            ...
            syncDefaultTranslationPanel(true);
    ...
    async function remove(id) {
        AF.Modal.confirm(t('table.actions.confirm_delete'), async () => {
            try {
                await AF.delete(`${API}/${id}`, { id: id, tenant_id: getTenantIdForApi() });
    ...
    function selectImage() {
        ...
        if (iframe) {
            ...
            iframe.src = `/admin/fragments/media_studio.php?embedded=1&tenant_id=${getTenantIdForApi()}&owner_id=${ownerId}&image_type_id=1&mode=select`;
        }
    ...
    function resetFilters() {
        ...
        if (el.tenantFilter) el.tenantFilter.value = getTenantIdForApi();
    ...
    async function init() {
        ...
        if (el.formName) el.formName.addEventListener('input', () => syncDefaultTranslationPanel(false));
        if (el.formSlug) el.formSlug.addEventListener('input', () => syncDefaultTranslationPanel(false));
        if (el.formDescription) el.formDescription.addEventListener('input', () => syncDefaultTranslationPanel(false));
    ...
    async function startExcelImport() {
        ...
        const tenantIdForApi = (window.APP_CONFIG && window.APP_CONFIG.TENANT_ID !== undefined && window.APP_CONFIG.TENANT_ID !== null) ? window.APP_CONFIG.TENANT_ID : '';
        let tenantId = tenantIdForApi;
        if (tenantId === '' && !window.PAGE_PERMISSIONS?.isSuperAdmin) tenantId = 1;
        ...
        for (let i = 0; i < sortedRows.length; i++) {
            ...
            const payload = normalizeTranslationsForSave(translations, {
                tenant_id:   tenantId,
                name:        name,
                slug:        slug,
                parent_id:   parentId,
