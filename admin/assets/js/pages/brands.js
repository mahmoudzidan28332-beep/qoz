...
(function () {
    ...
    const state = {
        ...
        isPlatformAdmin: !!CONFIG.isPlatformAdmin,
        brands: [],
    ...
    async function renderTable(items) {
        ...
        for (const item of items) {
            ...
            const tenantTd = state.isPlatformAdmin ? `<td>${item.tenant_id}</td>` : '';
    ...
    async function save(e) {
        ...
        const data = {
            tenant_id: parseInt(formData.tenant_id) || state.tenantId,
    ...
    async function load(page = 1) {
        try {
            ...
            state.page = page;
            const currentTenantId = (state.filters && state.filters.tenant_id !== undefined) 
                                    ? state.filters.tenant_id 
                                    : state.tenantId;
    ...
    function applyFilters() {
        ...
        if (el.tenantFilter) {
            ...
            if (t !== "") {
                state.filters.tenant_id = parseInt(t);
            } else if (state.isPlatformAdmin) {
                state.filters.tenant_id = 0;
            }
        }
        if (el.statusFilter) {
