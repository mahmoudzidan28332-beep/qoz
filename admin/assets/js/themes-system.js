...
(function() {
    ...
    const PLATFORM_TENANT_ID = CFG.PLATFORM_TENANT_ID || 1;
    const IS_PLATFORM_ADMIN = !!CFG.IS_PLATFORM_ADMIN;
    ...
    function init() {
        ...
        loadI18n();
        syncTargetSelectors();
        toggleAssignmentTenant();
        if (IS_PLATFORM_ADMIN) loadTenants();
        loadThemes();
    }
    ...
    function cacheElements() {
        ...
        el.targetFilter = $('themeTargetFilter');
        el.assignmentTarget = $('assignmentTarget');
        el.assignmentTenantId = $('assignmentTenantId');
        el.assignmentTenantWrap = $('assignmentTenantWrap');
        el.assignmentThemeId = $('assignmentThemeId');
        el.btnApplyActiveTheme = $('btnApplyActiveTheme');
        el.btnApplyDefaultTheme = $('btnApplyDefaultTheme');
        el.currentThemeSelectionSummary = $('currentThemeSelectionSummary');
        ...
        el.themeScope = $('themeScope');
        el.themeTarget = $('themeTarget');
    ...
    function bindEvents() {
        ...
        if (el.targetFilter) el.targetFilter.onchange = () => {
            syncTargetSelectors('filter');
            loadThemes();
            filterThemes();
        };
        if (el.assignmentTarget) el.assignmentTarget.onchange = () => {
            syncTargetSelectors('assignment');
            toggleAssignmentTenant();
            loadThemes();
        };
        if (el.assignmentTenantId) el.assignmentTenantId.onchange = () => loadThemes();
        if (el.btnApplyActiveTheme) el.btnApplyActiveTheme.onclick = () => applyThemeSelection('active');
        if (el.btnApplyDefaultTheme) el.btnApplyDefaultTheme.onclick = () => applyThemeSelection('default');
        // Form view
    ...
    async function loadThemes() {
        showLoading(true);
        try {
            const themeTarget = getCurrentThemeTarget();
            const actingTenantId = getActingTenantId();
            const ownerTenantId = IS_PLATFORM_ADMIN ? PLATFORM_TENANT_ID : TENANT_ID;
            const url = API.themes + '?tenant_id=' + actingTenantId + '&owner_tenant_id=' + ownerTenantId + '&theme_target=' + encodeURIComponent(themeTarget) + '&format=json';
            const res = await fetch(url);
            const json = await res.json();
            state.themes = extractItems(json);
            renderThemes(state.themes);
            renderAssignmentThemes();
    ...
    async function loadTenants() {
        if (!API.tenants || !el.assignmentTenantId) return;
        try {
            const res = await fetch(API.tenants + '?page=1&per_page=100');
            const json = await res.json();
            const data = json && json.data && Array.isArray(json.data.items) ? json.data.items : [];
            state.tenants = data;
            renderTenantOptions();
        } catch (e) {
            console.warn('[ThemesSystem] loadTenants failed:', e);
        }
    }
    ...
    function renderThemes(themes) {
        ...
        el.tableBody.innerHTML = themes.map(th => {
            ...
            const isActive = String(th.is_active) === '1' || th.is_active === 1 || th.is_active === true;
            const isDefault = String(th.is_default) === '1' || th.is_default === 1 || th.is_default === true;
            const canEdit = !!IS_PLATFORM_ADMIN || (th.theme_scope || 'tenant') === 'tenant';
            const actions = canEdit
                ? '<button class="btn btn-sm btn-primary" onclick="ThemesSystem.editTheme(' + th.id + ')">' +
                  ...
                  '</button> ' +
                  '<button class="btn btn-sm btn-secondary" onclick="ThemesSystem.activateTheme(' + th.id + ')" ' + (isActive ? 'disabled' : '') + '>' +
                    '<i class="fas fa-check"></i> Activate' +
                  '</button> ' +
                  '<button class="btn btn-sm btn-secondary" onclick="ThemesSystem.setDefaultTheme(' + th.id + ')" ' + (isDefault ? 'disabled' : '') + '>' +
            ...
            return '<tr>' +
                ...
                '<td>' + escapeHtml(th.theme_scope || 'tenant') + '</td>' +
                '<td>' + escapeHtml(th.theme_target || 'tenant_store') + '</td>' +
    ...
    function filterThemes() {
        ...
        const target = (el.targetFilter && el.targetFilter.value) || '';
        if (target) {
            filtered = filtered.filter(th => (th.theme_target || 'tenant_store') === target);
        }
        renderThemes(filtered);
    }

    function renderTenantOptions() {
        if (!el.assignmentTenantId) return;
        const selected = String(getActingTenantId());
        const tenants = state.tenants.length ? state.tenants : [{ id: TENANT_ID, name: 'Tenant #' + TENANT_ID }];
        el.assignmentTenantId.innerHTML = tenants.map(item => {
            const id = item.id;
            const name = escapeHtml(item.name || item.tenant_name || ('Tenant #' + id));
            return '<option value="' + id + '"' + (String(id) === selected ? ' selected' : '') + '>' + name + ' (#' + id + ')</option>';
        }).join('');
    }

    function renderAssignmentThemes() {
        if (!el.assignmentThemeId) return;
        const current = String(el.assignmentThemeId.value || '');
        const options = ['<option value="">Select theme</option>'].concat(
            state.themes.map(th => '<option value="' + th.id + '"' + (String(th.id) === current ? ' selected' : '') + '>' + escapeHtml(th.name || th.slug || ('Theme #' + th.id)) + '</option>')
        );
        el.assignmentThemeId.innerHTML = options.join('');
    }

    async function loadCurrentSelections() {
        if (!el.currentThemeSelectionSummary || !API.themes) return;
        const target = getCurrentThemeTarget();
        const tenantId = getActingTenantId();
        try {
            const [activeRes, defaultRes] = await Promise.all([
                fetch(API.themes + '/active?tenant_id=' + tenantId + '&theme_target=' + encodeURIComponent(target)),
                fetch(API.themes + '/default?tenant_id=' + tenantId + '&theme_target=' + encodeURIComponent(target))
            ]);
            const activeJson = await activeRes.json();
            const defaultJson = await defaultRes.json();
            const activeTheme = activeJson && activeJson.data ? activeJson.data : null;
            const defaultTheme = defaultJson && defaultJson.data ? defaultJson.data : null;
            el.currentThemeSelectionSummary.textContent =
                'Context: ' + target + (target === 'tenant_store' ? ' / Tenant #' + tenantId : '') +
                ' | Active: ' + (activeTheme && activeTheme.name ? activeTheme.name : 'None') +
                ' | Default: ' + (defaultTheme && defaultTheme.name ? defaultTheme.name : 'None');
        } catch (e) {
            el.currentThemeSelectionSummary.textContent = '';
        }
    }

    async function applyThemeSelection(mode) {
        const themeId = el.assignmentThemeId ? el.assignmentThemeId.value : '';
        if (!themeId) {
            showAlert('warning', 'Select a theme first');
            return;
        }
        if (mode === 'active') {
    ...
    function getCurrentThemeTarget() {
        return (el.assignmentTarget && el.assignmentTarget.value) || (el.targetFilter && el.targetFilter.value) || 'tenant_store';
    }

    function getActingTenantId() {
        if (IS_PLATFORM_ADMIN && el.assignmentTenantId && getCurrentThemeTarget() === 'tenant_store') {
            return parseInt(el.assignmentTenantId.value, 10) || PLATFORM_TENANT_ID;
        }
        return TENANT_ID || PLATFORM_TENANT_ID;
    }

    function toggleAssignmentTenant() {
        if (!el.assignmentTenantWrap) return;
        el.assignmentTenantWrap.style.display = IS_PLATFORM_ADMIN && getCurrentThemeTarget() === 'tenant_store' ? '' : 'none';
    }

    function syncTargetSelectors(source) {
        const value = source === 'assignment'
            ? ((el.assignmentTarget && el.assignmentTarget.value) || 'tenant_store')
            : ((el.targetFilter && el.targetFilter.value) || 'tenant_store');
        if (el.assignmentTarget) el.assignmentTarget.value = value;
        if (el.targetFilter) el.targetFilter.value = value;
    }
    ...
    function showForm(themeId) {
        ...
        if (el.themeScope) el.themeScope.value = IS_PLATFORM_ADMIN ? 'global' : 'tenant';
        if (el.themeTarget) el.themeTarget.value = (el.targetFilter && el.targetFilter.value) || 'tenant_store';
    ...
    function populateThemeForm(themeId) {
        ...
        if (el.themeScope) el.themeScope.value = theme.theme_scope || 'tenant';
        if (el.themeTarget) el.themeTarget.value = theme.theme_target || 'tenant_store';
    ...
    async function saveTheme() {
        ...
        const payload = {
            ...
            theme_scope: (el.themeScope && el.themeScope.value) || (IS_PLATFORM_ADMIN ? 'global' : 'tenant'),
            theme_target: (el.themeTarget && el.themeTarget.value) || 'tenant_store',
            owner_tenant_id: IS_PLATFORM_ADMIN ? PLATFORM_TENANT_ID : TENANT_ID,
    ...
    async function deleteTheme() {
        ...
        const themeTarget = (el.themeTarget && el.themeTarget.value) || ((el.targetFilter && el.targetFilter.value) || 'tenant_store');
        const ownerTenantId = IS_PLATFORM_ADMIN ? PLATFORM_TENANT_ID : TENANT_ID;

        try {
            const res = await fetch(API.themes + '?id=' + themeId + '&theme_target=' + encodeURIComponent(themeTarget) + '&owner_tenant_id=' + ownerTenantId, {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' }
            });
            const json = await res.json();
    ...
    async function removeTheme(themeId) {
        if (!confirm(t('theme_manager.messages.confirm.delete', 'Are you sure you want to delete this theme?'))) return;
        const theme = state.themes.find(th => String(th.id) === String(themeId)) || {};
        const themeTarget = theme.theme_target || ((el.targetFilter && el.targetFilter.value) || 'tenant_store');
        const ownerTenantId = IS_PLATFORM_ADMIN ? PLATFORM_TENANT_ID : TENANT_ID;
        try {
            const res = await fetch(API.themes + '?id=' + themeId + '&theme_target=' + encodeURIComponent(themeTarget) + '&owner_tenant_id=' + ownerTenantId, {
                method: 'DELETE',
    ...
    async function activateTheme(themeId) {
        ...
        if (!theme) return;
        try {
            const res = await fetch(API.themes + '/activate?tenant_id=' + getActingTenantId() + '&theme_target=' + encodeURIComponent(theme.theme_target || getCurrentThemeTarget()) + '&owner_tenant_id=' + (IS_PLATFORM_ADMIN ? PLATFORM_TENANT_ID : TENANT_ID), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            const json = await res.json();
            if (json.success) {
                showAlert('success', 'Theme activated');
                await loadThemes();
            } else {
                showAlert('error', json.message || 'Failed to activate');
            }
        } catch (e) {
    ...
    async function setDefaultTheme(themeId) {
        ...
        if (!theme) return;
        try {
            const res = await fetch(API.themes + '/set-default?tenant_id=' + getActingTenantId() + '&theme_target=' + encodeURIComponent(theme.theme_target || getCurrentThemeTarget()) + '&owner_tenant_id=' + (IS_PLATFORM_ADMIN ? PLATFORM_TENANT_ID : TENANT_ID), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ slug: theme.slug })
            });
            const json = await res.json();
            if (json.success) {
                showAlert('success', 'Default theme updated');
                await loadThemes();
            } else {
                showAlert('error', json.message || 'Failed to set default');
            }
        } catch (e) {
            showAlert('error', 'Failed to set default');
        }
    }
    ...
    function clearAllSettingsLists() {
        ['design', 'color', 'font', 'button', 'card', 'system'].forEach(type => {
            const listEl = getSettingsListEl(type);
    ...
    function hideAllSettingForms() {
        ['design', 'color', 'font', 'button', 'card', 'system'].forEach(prefix => {
            const form = document.getElementById(prefix + 'Form');
            if (form) form.style.display = 'none';
        });
    }
    ...
    function editSetting(type, itemId) {
        ...
        const tabMap = { design: 'design', color: 'colors', font: 'fonts', button: 'buttons', card: 'cards', system: 'system' };
