...
final class JobInterviewsValidator
{
    ...
    public function validate(array $data, bool $isUpdate = false): void
    {
        ...
        if (isset($data['application_id']) && (!is_numeric($data['application_id']) || $data['application_id'] <= 0)) {
            throw new InvalidArgumentException("application_id must be a positive integer.");
        }
        if (isset($data['interview_type']) && !in_array($data['interview_type'], self::INTERVIEW_TYPES, true)) {
            throw new InvalidArgumentException("Invalid interview_type. Allowed: " . implode(', ', self::INTERVIEW_TYPES));
        }

        $this->validateDateAndDuration($data, $isUpdate);
        $this->validateContactFields($data);
        ...
        if (isset($data['rating']) && $data['rating'] !== null && (!is_numeric($data['rating']) || $data['rating'] < 1 || $data['rating'] > 5)) {
            throw new InvalidArgumentException("rating must be between 1 and 5.");
        }
        if (isset($data['created_by']) && $data['created_by'] !== null && (!is_numeric($data['created_by']) || $data['created_by'] <= 0)) {
            throw new InvalidArgumentException("created_by must be a positive integer.");
        }
        if (isset($data['interview_type'])) { $this->validateByType($data['interview_type'], $data); }
    }

    private function validateDateAndDuration(array $data, bool $isUpdate): void
    {
        ...
        if (isset($data['interview_duration']) && $data['interview_duration'] !== null) {
            ...
            if ($data['interview_duration'] > 480) {
                throw new InvalidArgumentException("interview_duration cannot exceed 480 minutes (8 hours).");
            }
        }
    }

    private function validateContactFields(array $data): void
    {
        if (isset($data['location']) && $data['location'] !== null && strlen($data['location']) > 500) {
        ...
        if (isset($data['meeting_link']) && $data['meeting_link'] !== null && $data['meeting_link'] !== '') {
            if (strlen($data['meeting_link']) > 500) { throw new InvalidArgumentException("meeting_link must be at most 500 characters."); }
            if (!filter_var($data['meeting_link'], FILTER_VALIDATE_URL)) { throw new InvalidArgumentException("meeting_link must be a valid URL."); }
        }
        if (isset($data['interviewer_name']) && $data['interviewer_name'] !== null && strlen($data['interviewer_name']) > 255) {
        ...
        if (isset($data['interviewer_email']) && $data['interviewer_email'] !== null && $data['interviewer_email'] !== '') {
            if (strlen($data['interviewer_email']) > 191) { throw new InvalidArgumentException("interviewer_email must be at most 191 characters."); }
            if (!filter_var($data['interviewer_email'], FILTER_VALIDATE_EMAIL)) { throw new InvalidArgumentException("Invalid interviewer_email format."); }
        }
    }
